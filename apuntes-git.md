...
luego de haber configurado todo pe, naturalmente basta con hacer
...
si se esta trabajando en un proyecto importante donde se usan claves privadas o durante el desarrollo del proyecto se instalaron depenencias o existen carpetas de compilacion se recomienda usar .gitignore para evitar subir informacion sensible, cargar archivos innecesarios como las depenencias o archivos temporales. por lo tanto, dentro de la carpeta del repositorio hacer
...
Una vez logeado y enlazado git con github podemos crear un repositorio:
...
git add .
git commit -m "primer commit"
gh repo create nombre-proyecto --source=. --remote=origin # podemos usar la carpeta como fuente a la hora de crear el repositorio, tambien reemplaza usar git remote add origin
git branch -M main 
```
